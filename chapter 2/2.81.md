...
Suppose you install the mandatory procedure complex->complex to itself for complex, as modified by Louis.When the exp operation does not exist, as in the following statement

``` Scheme
(exp (make-complex-from-real-imag 10 0) (make-complex-from-real-imag 10 0))
```

Make the program loop.Add some information to apply-generic, and print it over and over again
...
....
```
As you can see, the modified program repeatedly calls' apply-generic 'with the same parameters, an endless loop.

### b)

Louis did not correct the same type of parameter coercion problem, but made it worse.When complex->complex is not added, the program will exit directly with an error message

```
No method for these types -- APPLY-GENERIC (exp (complex complex))
```

Louis's changes would instead create an endless loop.

This is modified. The get-coercion 'complex 'complex can be found in the table. Then the condition t1->t2 in cond will be true.Then execute

``` Scheme
(apply-generic op (t1->t2 a1) a2)
t1->t2 returns itself, so t1->t2 a1 is a1.So it's going to be a constant loop
...
Before the change, complex's mandatory function could not be found in the table, and the program exited with an error message.

### c)

Add a judgment for apply-generic.
...
; apply-generic causes an endless loop
;(define (apply-generic op . args)
