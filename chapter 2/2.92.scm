#lang racket
;The key to see poly->poly implementation, the polynomial into other variables, such as a polynomial
;(poly real - > poly real p 'x), converts the x variable, can be seen as (y + 1) * x ^ 2 + (y ^ 2 + 1) * x + y (1)
;(poly real - > poly real p 'y), convert y variable, can be seen as (x) * y ^ 2 + y + (x ^ 2 + 1) * (x ^ 2 + x - 1)
;The two are the same, mainly the way to look at the generation is different.
;Then using poly->poly function, you can convert the polynomials in add-poly and sub-poly into the same variable.

(require "ch2support.scm")

;; To avoid introducing too much code, numbers and polynomials are treated specially
...
;For example, y plus 1 times x squared, where x was the variable, I want to change it to y
;It can be broken down into two parts
;p1 = 1 * y^1 + 1 * y^0
;p2 = (x^2) * y^0
;And then you multiply the two parts p1 times p2 and you get the result
...
;Let's say x squared is going to change to y.You need to call (make-oder-poly 2 'x 'y)
;The new polynomial is of the form (1 * x ^ 2) * (y ^ 0), which is a nested structure
;oder = 0, coeff = (1 * x ^ 2), coeff itself is polynomial in x
      ...
      (let ((t1 (first-term L1))
        ...
        (if (> (order t2) (order t1))
            ...
            (let ((new-c (div (coeff t1) (coeff t2)))
              ...
              (if (=zero? new-c)  ;; We need to judge the case where the quotient is zero (which may happen if coeff is also polynomial), otherwise it will loop indefinitely
                  (list (the-empty-termlist) L1)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Print polynomials in infix mode for easy viewing of results
