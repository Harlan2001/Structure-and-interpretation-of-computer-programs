...
;The text has been given an implementation of sparse representation combined with the dense representation in [Exercise 2.89].
;The general function can be extracted so that the polynomial calculation can switch between the two representations respectively.
;To use both representations, you need a mechanism to pick which one is most appropriate.Two general functions have been added for this purpose
;* nonzero-term-ratio is used to calculate the proportion of non-zero items in the term-list
;* trans-termlist is used to convert freely between the two representations.
;Now you can modify the make-poly function.If there are more non-0 items (more than 0.5), it is converted to dense representation, otherwise it is converted to sparse representation.
;Because the general function masks the details of the specific representation, the two representations can also be interleaved.
...
(define (trans-termlist term-list type)
  ...
  ;;type is the same, no conversion is necessary
