The runtime environment of the scheme is included, including a simple gcse.
...
void MemGC_Flip(MemGC *gc) {
    ...
    // Expand the GC space dynamically
...
struct Scheme {
    ...
    int nullValue;                           // Reuse nullValue
    DisplayPrimitive displayPrimitiveHander; // For printing native procedures
    struct {
        void *userdata;
...
static Value *Scheme_AllocValue(Scheme *s, int *pidx) {
    ...
    if (valIdx < 0) {
        // The allocation fails, triggering the GC process
...
Reg *procedure(Reg *reg, Reg *lst) {
    ...
    reg->val = valIdx; // Finally set, prevent reg and lst are the same
    return reg;
}
...
Reg *cons(Reg *reg, Reg *car, Reg *cdr) {
    ...
    reg->val = valIdx; // Last set, prevent reg is the same as car or cdr
    return reg;
}
